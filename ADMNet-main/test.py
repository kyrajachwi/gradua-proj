...
def save_output(image_name, pred, d_dir):
    ...
    t = image_name.split("/")[-1].split(".")  # 修复了这里，使用文件名而不是整个路径
    t = t[0:-1]
    imidx = t[0]
    for i in range(1, len(t)):
        imidx = imidx + "." + t[i]

    output_path = os.path.join(d_dir, imidx + '_Ours.png')
    imo.save(output_path)


if __name__ == '__main__':
    # --------- Define the address and image format ---------
    image_dir = r"D:\Desktop\study_life\gradua-proj\DUTS\DUTS-TR\DUTS-TR-Image"
    prediction_dir = r"D:\Desktop\study_life\gradua-proj\DUTS\DUTS-TR\DUTS-TR-Mask"
